...
const AVAILABILITY_OPTIONS = [ 0, 10 ];

const normalizeAvailability = (value: number) => AVAILABILITY_OPTIONS.includes(value) ? value : 0;

export const WiredVariableFurniView: FC<{}> = props =>
{
    ...
    useEffect(() =>
    {
        ...
        setAvailability(normalizeAvailability(variable.persistence ?? 0));
    ...
    return (
        <WiredVariableBaseView save={ save }>
            ...
            <WiredControlSection title={ LocalizeText('wiredfurni.params.variables.settings') }>
                <WiredCheckbox checked={ hasValue } onChange={ setHasValue } label={ LocalizeText('wiredfurni.params.variables.settings.has_value') } />
            </WiredControlSection>
