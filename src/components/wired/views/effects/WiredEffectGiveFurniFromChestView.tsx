...
const ITERATION_RANDOM = 0;
const ITERATION_FIFO = 1;
const ITERATION_LIFO = 2;

interface FurniFromChestData extends ChestVariableData
{
...
const normalizeIterationMode = (value: number) =>
    value === ITERATION_RANDOM || value === ITERATION_LIFO ? value : ITERATION_FIFO;

export const WiredEffectGiveFurniFromChestView: FC<{}> = () =>
{
    ...
    const [ iterationMode, setIterationMode ] = useState(ITERATION_FIFO);
    ...
    const data = useMemo(() => parseWiredData<FurniFromChestData>(trigger?.stringData ?? ''), [ trigger ]);
    ...
    useEffect(() =>
    {
        ...
        setIterationMode(normalizeIterationMode(intData[6] ?? data.iterationMode ?? ITERATION_FIFO));
        setChestSource(normalizeFurniSource(intData[7] ?? data.chestSource ?? WIRED_SOURCE_SELECTED));
        setUserSource(normalizeUserSource(intData[8] ?? data.userSource ?? 0));
    ...
    return (
        <WiredEffectBaseView
            ...
            <WiredParam titleKey="wiredcontracts.reward_contract.reward_popup">
                <WiredTextarea
                    maxLength={ 200 }
                    value={ rewardText }
            ...
            <WiredParam titleKey="wiredfurni.params.chest_iteration_type" divider={ false }>
                <WiredSelect
                    value={ iterationMode }
                    onChange={ event => setIterationMode(normalizeIterationMode(Number(event.target.value))) }
                    options={ [
                        { value: ITERATION_RANDOM, label: LocalizeText('wiredfurni.params.chest_iteration_type.0') },
                        { value: ITERATION_FIFO, label: LocalizeText('wiredfurni.params.chest_iteration_type.1') },
                        { value: ITERATION_LIFO, label: LocalizeText('wiredfurni.params.chest_iteration_type.2') }
                    ] } />
            </WiredParam>
        </WiredEffectBaseView>
    );
