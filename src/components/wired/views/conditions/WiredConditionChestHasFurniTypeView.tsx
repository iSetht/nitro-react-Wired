...
interface ChestHasFurniTypeData extends ChestVariableData
{
    amount?: number;
...
export const WiredConditionChestHasFurniTypeView: FC<{}> = () =>
{
    ...
    const data = useMemo(() => parseWiredData<ChestHasFurniTypeData>(trigger?.stringData ?? ''), [ trigger ]);
    ...
    const needsFurniSelection = isSelectedFurniSource(chestSource) ||
        isSelectedFurniSource(itemTypeSource) ||
    ...
    useEffect(() =>
    {
        ...
        setItemTypeSource(normalizeFurniSource(intData[6] ?? data.itemTypeSource ?? WIRED_SOURCE_SELECTED));
        setAmountVariableSource(normalizeVariableSource(savedVariableType, intData[7] ?? data.amountVariableSource ?? savedVariableType));
    ...
    return (
        <WiredConditionBaseView
            ...
            advancedSlot={ <WiredConditionQuantifierSection kind="furni" name="chestHasFurniTypeQuantifier" value={ quantifier } onChange={ setQuantifier } /> }
            ...
            <WiredParam titleKey="wiredfurni.params.choose_type">
                <WiredRadioGroup
                    inline
                    gap={ 2 }
                    name="chestHasFurniTypeComparison"
                    value={ comparison }
            ...
            <WiredParam titleKey="wiredfurni.params.variables.compare_value" divider={ false }>
                <WiredValueOrVariable
                    radioName="chestHasFurniTypeAmount"
