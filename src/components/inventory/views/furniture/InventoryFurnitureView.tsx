...
export const InventoryFurnitureView: FC<InventoryFurnitureViewProps> = props =>
{
    ...
    // First filter: remove clothing + specific hidden items
    const groupItems = allGroupItems.filter(group =>
{
    ...
    // Exclude rares
    if(furniData.furniLine === 'rare') return false;

    // Exclude clothing
    if(className.startsWith('clothing_')) return false;

    return true;
});


    // Sync filteredGroupItems whenever groupItems changes
    useEffect(() =>
    {
    ...
    useEffect(() =>
    {
        ...
        if((furnitureItem.category === FurniCategory.WALL_PAPER) ||
            ...
            <Column size={ 5 } overflow="auto">
                ...
                { selectedItem &&
                    <Column grow justifyContent="between" gap={ 2 }>
                        ...
                        <Column gap={ 1 }>
                            { !!roomSession &&
                                <Button variant="success" onClick={ () => attemptItemPlacement(selectedItem) }>
                                    { LocalizeText('inventory.furni.placetoroom') }
                                </Button>
                            }

                            { selectedItem.isSellable &&
                                <Button onClick={ () => attemptPlaceMarketplaceOffer(selectedItem) }>
